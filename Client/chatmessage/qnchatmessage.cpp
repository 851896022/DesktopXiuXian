...
QNChatMessage::QNChatMessage(QWidget *parent,QString name,QString head) : QWidget(parent)
{
    m_userName = name;
...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    {
        m_leftPixmap = Base64_To_Image(head.toLatin1());
        m_rightPixmap = Base64_To_Image(head.toLatin1());
    }
...
QSize QNChatMessage::fontRect(QString str)
{
    ...
    m_sanjiaoLeftRect = QRect(iconWH+iconSpaceW+iconRectW, m_lineHeight/2+15, sanJiaoW, hei - m_lineHeight);
    m_sanjiaoRightRect = QRect(this->width() - iconRectW - iconWH - iconSpaceW - sanJiaoW, m_lineHeight/2, sanJiaoW, hei - m_lineHeight);

    if(size.width() < (m_textWidth+m_spaceWid)) {
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_lineHeight/4*3+7, size.width()-m_spaceWid+2*textSpaceRect, (hei-m_lineHeight));
        ...
        m_kuangLeftRect.setRect(m_sanjiaoLeftRect.x()+m_sanjiaoLeftRect.width(), m_lineHeight/4*3+7, m_kuangWidth, (hei-m_lineHeight));
    ...
    m_textLeftRect.setRect(m_kuangLeftRect.x()+textSpaceRect,m_kuangLeftRect.y()+iconTMPH,
                           m_kuangLeftRect.width()-2*textSpaceRect,m_kuangLeftRect.height()-2*iconTMPH+5);

    m_nameLeftRect.setRect(m_kuangLeftRect.x(),iconTMPH-5,
                           width(),15);
...
//        painter.drawRoundedRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
        ...
        //昵称
        {
            ...
            painter.drawText(m_nameLeftRect, m_userName,option);
...
#include <QBuffer>
QPixmap QNChatMessage::Base64_To_Image(QByteArray bytearray)
{
